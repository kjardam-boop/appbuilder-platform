...
/**
 * Apply theme tokens as CSS variables for live preview
 */
function applyThemePreview(tokens: any) {
  ...
  if (tokens.primary) root.style.setProperty('--primary', hexToHSL(tokens.primary));
  if (tokens.accent) root.style.setProperty('--accent', hexToHSL(tokens.accent));
  if (tokens.secondary) root.style.setProperty('--secondary', hexToHSL(tokens.secondary));
  if (tokens.surface) root.style.setProperty('--card', hexToHSL(tokens.surface));
  if (tokens.textOnSurface) root.style.setProperty('--foreground', hexToHSL(tokens.textOnSurface));
  if (tokens.destructive) root.style.setProperty('--destructive', hexToHSL(tokens.destructive));
  if (tokens.success) root.style.setProperty('--success', hexToHSL(tokens.success));
  if (tokens.warning) root.style.setProperty('--warning', hexToHSL(tokens.warning));
  if (tokens.muted) root.style.setProperty('--muted', hexToHSL(tokens.muted));
  if (tokens.fontStack) root.style.setProperty('--font-sans', tokens.fontStack);
}
...
export const TenantBranding = () => {
  ...
  // Apply theme preview in real-time when tokens change
  useEffect(() => {
    if (editedTokens) {
      applyThemePreview(editedTokens);
    }
  }, [editedTokens]);
