...
export function ContactPersonsCard({
  ...
  const handleSaveContact = async (contact: ContactPerson) => {
    try {
      ...
      if (editingContact) {
        ...
        const index = contacts.findIndex(c => c.full_name === editingContact.full_name && c.title === editingContact.title);
  ...
  return <>
      <Card>
        ...
        <CardContent>
          {contacts.length === 0 ? <div className="text-center py-6">
              <p className="text-sm text-muted-foreground">
                Ingen kontaktpersoner registrert ennå
              </p>
            </div> : <div className="space-y-3">
              {contacts.map((contact, index) => <div key={index} className="p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors group">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-medium">{contact.full_name}</span>
                        {contact.is_primary && <Badge variant="secondary" className="text-xs">
                            Primær
                          </Badge>}
                      </div>
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <Button size="icon" variant="ghost" className="h-7 w-7 text-destructive" onClick={() => handleDeleteContact(index)}>
                        <Trash2 className="h-3 w-3" />
                      </Button>
                    </div>
                  </div>
                  <div className="space-y-1">
                    {contact.email && <a href={`mailto:${contact.email}`} className="flex items-center gap-2 text-sm text-primary hover:underline">
                        <Mail className="h-3 w-3" />
                        {contact.email}
                      </a>}
                    {contact.phone && <a href={`tel:${contact.phone}`} className="flex items-center gap-2 text-sm text-primary hover:underline">
                        <Phone className="h-3 w-3" />
                        {contact.phone}
                      </a>}
                  </div>
                  {contact.notes && <p className="text-xs text-muted-foreground mt-2 italic">
                      {contact.notes}
                    </p>}
                </div>)}
            </div>}
