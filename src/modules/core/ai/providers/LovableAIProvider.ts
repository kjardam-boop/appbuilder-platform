/**
 * Lovable AI Provider
 * Uses platform-level Lovable AI Gateway (default fallback)
 */
...
export class LovableAIProvider extends BaseAIProvider {
  ...
  protected getHeaders(): Record<string, string> {
    // Platform-level key is handled by edge function environment
    return {
  ...
  async testConnection(): Promise<boolean> {
    // Always true for platform-level provider
    return true;
  }
}
